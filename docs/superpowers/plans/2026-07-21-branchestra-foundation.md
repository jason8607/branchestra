...
**Tech Stack:** Node.js 24.18.0, pnpm 11.15.1, Electron 43.1.1, electron-vite 5.0.0, React/React DOM 19.2.7, Vite 8.1.5, `@vitejs/plugin-react` 6.0.3, TypeScript 7.0.2, Zod 4.4.3, `node:sqlite` `DatabaseSync`, Vitest 4.1.10, and Playwright 1.61.1.
...
|---|---|
| `.nvmrc`, `.gitignore`, `package.json`, `pnpm-lock.yaml` | Reproducible single-package runtime, scripts, and exact dependency graph. |
...
Expected: pnpm reports version `11.15.1`, installs only the exact direct versions above (including Renderer test and lint tooling), and creates `pnpm-lock.yaml`.
...
```bash
git add .nvmrc .gitignore package.json pnpm-lock.yaml electron.vite.config.ts eslint.config.mjs tsconfig.json tsconfig.node.json tsconfig.renderer.json vitest.config.ts playwright.config.ts src/main/window-options.ts src/main/index.ts src/preload/index.ts src/renderer/index.html src/renderer/main.tsx src/renderer/App.tsx src/renderer/styles.css tests/unit/window-options.test.ts
