...
const request = {
  runId: "019f842d-e19a-7cc1-9d73-4d287bf40558", taskId: "task-1", roomId: "room-1", provider: "codex" as const,
  role: "lead" as const, worktreePath: "/tmp/worktree", instruction: "implement", contextVersion: 1,
  contextHash: `sha256:${"a".repeat(64)}`, checkpointOid: null,
  ...
  it("requires ready health, sends a strict runner command, and streams normalized events", async () => {
    ...
    const adapter = new RunnerBackedAdapter({
      ...
      normalize: (raw, run) => (raw as { type?: string }).type === "thread.started"
        ? [{ ...run, provider: "codex", type: "session.started", sessionId: "thread-1" }]
        : [{ ...run, provider: "codex", type: "run.completed", result: "done" }],
    ...
    expect(send).toHaveBeenCalledWith(expect.objectContaining({
      request: expect.objectContaining({ contextHash: "a".repeat(64) })
    }));
    await deliver!({ type: "provider.raw", runId: request.runId, providerSeq: 1, receivedAt: "2026-07-21T10:00:01.000Z", payload: {} });
